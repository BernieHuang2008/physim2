...
function init_pg_about() {
    pg_about_dom.innerHTML = `
        <div class="pg-about-leftbar">
            ...
            <li data-pageurl="/pages/about.html" class="default-page">${t("About")}</li>
            <li data-pageurl="/pages/settings.html">${t("Settings")}</li>
        </div>
        <div class="pg-about-content">
        <iframe id="pg-about-iframe" class="pg-about-iframe"></iframe>
        </div>
    `;
    ...
    leftbar_dom.addEventListener("click", (e) => {
        ...
        // ui feedback
        $$(".pg-about-leftbar li.selected").forEach(li => li.classList.remove("selected"));
        e.target.classList.add("selected");

        // switch page
        const pageurl = e.target.getAttribute("data-pageurl");
        if (pageurl) {
            switch_page(pageurl);
        }
    });
...
function switch_page(page_url) {
    $("#pg-about-iframe").src = page_url;
}


function pg_about_show() {
    pg_about_dom.querySelector(".default-page").click();
